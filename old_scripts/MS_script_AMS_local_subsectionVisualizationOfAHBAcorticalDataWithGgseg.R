...
## MS_script_AMS_local - subsection "Visualization of AHBA cortical data with ggseg"
...
# ....... no parcellation for accumbens area?
...
# None of the regions had stat. significantly higher young OT genes cortical expression values than the expression of the average brain. 10% of the 100 random 
# young gene sets of equal size had one or two significant regions. 

# We found that a number of the statistically significant effects are specific to the oxytocin pathway and not due to the random association of the SNP in the 
# gene-set with the chosen phenotypes
...
## VISUALIZATION FOR EACH YOUNG OT GENE SEPARATELY ##

## cortical expression 

# ...... per gene
list_g_yc <- colnames(OTmod_c[5:23])
list_g_yc  

exp_yc <- function(gene) {
  yc <- ggseg(.data=OTmod_c, mapping=aes_string(fill=gene), colour="white", size=1, position="stacked", hemisphere="left" ) +
    theme_void()
  
  yc <- yc + viridis::scale_fill_viridis(option = 'turbo') + 
  ...
  attr(value, "class") <- "exp_yc"
  value
}


res_yc <- sapply(list_g_yc, exp_yc, USE.NAMES = T)
res_yc[["OXTR.yc"]]

## plotting 
ygene_ccom <- plot_grid(plotlist = res_yc, ncol=4, scale = 0.9)
ygene_ccom

# save plot
ggsave(filename = paste0(BASE, "RProject/OT-bd-analyses_loc/output/ygene_ccom.pdf"), ygene_ccom,
       width = 35, height = 20, units = "in", device='pdf')
...
# ....... per gene
list_g_ysc <- colnames(OTmod_sc[5:23])
list_g_ysc  

exp_ysc <- function(gene) {
  ysc <- ggseg(.data=OTmod_sc, atlas="aseg", mapping=aes_string(fill=gene), colour="white", size=1) +
    theme_void()
  
  ysc <- ysc + viridis::scale_fill_viridis(option = 'turbo', discrete = F) + 
    labs(title=paste(gene, "subcortical expression"), subtitle = "axial | sagittal", fill="mRNA intensity") 
  
  ysc <- ysc + theme(plot.title = element_text(hjust=0.5, size=16),
  ...
  attr(value, "class") <- "exp_ysc"
  value
}

res_ysc <- sapply(list_g_ysc, exp_ysc, USE.NAMES = T)
res_ysc[["OXTR.ysc"]]

## plotting 
ygene_sccom <- plot_grid(plotlist = res_ysc, ncol=4, scale = 0.9)
ygene_sccom

# save plot
ggsave(filename = paste0(BASE, "RProject/OT-bd-analyses_loc/output/ygene_sccom.pdf"), ygene_sccom,
       width = 35, height = 20, units = "in", device='pdf')
...
# combine plot for full young OT pathway genes cortical and subcortical
geneset_ycysc <- plot_grid(genset_yc, genset_ysc, nrow=2)
geneset_ycysc

# save plot
ggsave(filename = paste0(BASE, "RProject/OT-bd-analyses_loc/output/geneset_ycysc.pdf"), geneset_ycysc,
