...
# rm(list = ls()) # USE WITH CAUTION, delete all objects in the workspace
# gc(reset = T)   # USE WITH CAUTION, resest memory (especially useful when working with large data sets)
...
library(data.table)    # fread(), to load dnds ratios
library(gage)          # install with BiocManager (Bioconductor), for kegg.gsets() function
library(KEGGREST)      # install with BiocManager (Bioconductor), for keggGet() function

# data visualization
library(ggrepel)       # for manually labeling data points in plots, geom_label_repel()
...
library(ape)           # read phylogenetic tree template
...
# save plot
ggsave(filename = "output/supp_mat_new/sup_mat_12a.pdf", dnds_OT_tree_hq,
...
# save plot
ggsave(filename = "output/supp_mat_new/sup_mat_12b.pdf", dnds_OT_tree_lq,
      width = 20, height = 8, units = "in", device = 'pdf')
...
# save plot
ggsave(filename = "output/supp_mat_new/sup_mat_05.pdf", dnds_ALL_tree_mq,
