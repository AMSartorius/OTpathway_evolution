...
library(data.table)      # fread() function, load FUMA output
library(stringr)         # str_replace_all(), replace all occurrences of a matched pattern in a string
library(tidyverse)       # for everything and pipes
library(writexl)         # export .xlsx files 
library(cowplot)         # arrange figures in a grid
...
ggsave(filename = "output/figures/FUMA_tissueEnrich01_b.pdf", p.df.medage.mod,
...
# ggsave(filename = "output/supp_mat_new/FUMA_tissueEnrich02.pdf", p.df.ancmedagemod,
