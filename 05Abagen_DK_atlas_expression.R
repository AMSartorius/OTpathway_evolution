...
###############
# IMPORTANT !!! The package "powerAnalysis" has to be downloaded from the archive: https://cran.r-project.org/src/contrib/Archive/powerAnalysis/, version 0.2.1
###############
...
library(ggseg)           # dk atlas, cortical expression visualisation
# library(ggsegDefaultExtra)
library(ggridges)        # geom_density_ridges() function
library(cowplot)         # plot_grid(), arrange figures in a grid
...
for (i in pat.dk) {
  list.dk1[[i]] <- read_csv(paste0("data/processed/abagen_dk/AHBAdk_220513_", i, ".csv"))  # fetch original files with all genes
}
...
# save figure
ggsave(filename = "output/figures/abagen_analysis01_ScC_combined02.pdf", g222_p127,
...
for (i in pat.dk) {
  list.dk1[[i]] <- read_csv(paste0("data/processed/abagen_dk/AHBAdk_220513_", i, ".csv"))
}
...
# ....... for supplementary materials
write_xlsx(g.means.lt.sc, "output/supp_mat_new/sup_mat_08.xlsx")
...
# ...... for supplementary materials
write_xlsx(test.stats.dk2.desc, "output/supp_mat_new/sup_mat_11.xlsx")
...
# export figure
ggsave(filename = "output/figures/abagen_analysis02_cSc_03.pdf", lpp,
