...
# get gene symbols/names for all genes in the genome but only those that are available in the phylo map 
genesid <- ensembldb::select(EnsDb.Hsapiens.v79, 
                              keys    = HomoSapiens.PhyloMap2013$EnsemblGeneID, 
                              keytype = "GENEID", 
                              columns = c("SYMBOL","GENEID"))
genesid <- genesid %>% distinct(SYMBOL, .keep_all = TRUE)                          

# join the two lists to annotate list of gene symbols with phylostrata
genesid_ps <- dplyr::inner_join(HomoSapiens.PhyloMap2013, genesid, by = c("EnsemblGeneID" = "GENEID")) # 19607 genes
## done
...
yOT <- genesid_ps[genesid_ps$EnsemblGeneID %in% yOTids, ]  # annotate with gene symbols
...
dkD1_y <- dkD1[, names(dkD1) %in% c("label", yOT$SYMBOL)]                # extract subset of modern OT genes, 20 modern genes, 83 regions, lateralized 
...
#              lower expressed in 2 sub-cortical regions.
...
library(writexl)
write_xlsx(DKresdat_l, paste0(BASE, "supp_mat02b.xlsx"))
...
# fetch gene colnames
cnames <- colnames(dkD1_y[,c(2:21)])           # any df will work, because they have the same gene colnames
e_list <- list()
for (i in cnames) {
...
g_means_lt_sc$mean2 <-  rowMeans(g_means_lt_sc[5:24])                                                    # calculate means manually (sanity check)
# --> identical :) Phew.


# ....... for supplementary materials
#write_xlsx(g_means_lt_sc, paste0(BASE, "supp_mat02a.xlsx"))
...
ytext <- c("Thalamus proper", "Brain stem",  "Pallidum", "Accumbens area", "Putamen", "Hippocampus", "Caudate", "Amygdala")
...
g222 <- g222 + geom_vline(xintercept = mean(meanS_dklL$mean_allstruc), linetype="dashed", size=.75)      # add mean line for all regions, cortical and sub-cortical
...
#ggsave(filename = paste0(BASE, "RProject/OT-bd-analyses_loc/output/Ana1_subcortical_vert_2013.pdf"), g222,
...
#ggsave(filename = paste0(BASE, "RProject/OT-bd-analyses_loc/output/Ana1DK_ScC_combined_2013.pdf"), g222_p127,
#       width = 15, height = 10.5, units = "in", device='pdf')



# check whether genes from certain phylos are increased or decreased in their expression
...
test_phyloup <- subset(test, grepl(patstructures, test$label))
...
# ...... for supplementary materials
#write_xlsx(test_stats_dk2_o, paste0(BASE, "supp_mat03.xlsx"))
...
lpp <- lpp + 
     annotate('text', x = 36, y = 0.4395165, label = '*', colour= "black", size=14) +
     scale_y_continuous(breaks = c(.45, .475, .5, .525, .55))

lpp

ggsave(filename = paste0(BASE, "RProject/OT-bd-analyses_loc/output/Ana2DK_cSc2013b.pdf"), lpp,
